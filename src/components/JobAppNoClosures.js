...
function JobApp() {
  ...
  function testKeyWordIsIncludedInJob(keyWord, job) {
    return (
      keyWord === job.role ||
      keyWord === job.level ||
  ...
  function testJobIncludesAllKeyWords(keyWords, job) {
    return keyWords.every((keyWord) =>
      testKeyWordIsIncludedInJob(keyWord, job)
    );
  }

  const jobsToShow = allJobs.filter((job) =>
    testJobIncludesAllKeyWords(keyWords, job)
  );
